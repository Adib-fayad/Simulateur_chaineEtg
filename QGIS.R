...
# 1. On charge les données des tableaux purs
...
# 2. On "POMPE" LES STYLES (Formulaires QGIS) depuis les fichiers sources
...
for (fichier_cible in liste_gpkg_finis) {
  ...
  # A) Écriture des données (génère la clé primaire "fid" pour rendre éditable)
  ...
  # B) Injection des formulaires dans la base SQLite
  con_cible <- dbConnect(SQLite(), fichier_cible)
  # dbAppendTable rajoute nos 2 lignes à la suite des styles Étangs et OS créés en Boucle 1 !
  dbAppendTable(con_cible, "layer_styles", styles_tableaux)
  dbDisconnect(con_cible)
}

print("✅ Opération terminée avec succès ! Tableaux éditables AVEC listes déroulantes et calendriers.")
