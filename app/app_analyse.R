    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("Analyse Évaporation (P - ETP)",
                 br(),
                 h4("Analyse Statistique des Pertes d'Eau en Volume (m³)"),
                 p("Ce graphique compare directement les volumes d'eau perdus mesurés sur le terrain (Sonde) avec les pertes calculées par les modèles (Évaporation + Fuites) en période sèche."),
                 fluidRow(
                 ...
                 hr(),
                 h4("Statistiques des pertes sur la période sélectionnée"),
                 DTOutput("table_evap_resume")
        ),
        
        # --- ONGLET 8 : BILAN CHRONOLOGIQUE GLOBAL ---
        tabPanel("Bilan Chronologique Global",
                 br(),
                 h4("Évolution globale du stock de la chaîne d'étangs (en mm)"),
                 p("Ce chronogramme compare le stock d'eau (converti en mm sur le bassin versant) par rapport à la capacité maximale et au besoin réel. La pluie (RR) est superposée en arrière-plan."),
                 fluidRow(
                   column(4, dateRangeInput("dates_bilan_global", "Période d'analyse :", 
                                            start = "2015-01-01", end = "2023-12-31", width = "100%")),
                   column(4, selectInput("time_step_bilan", "Pas de temps (Lissage) :", 
                                         choices = c("Journalier (Brut)" = "day", "Hebdomadaire (7 jours)" = "week", "Mensuel" = "month", "Annuel" = "year"), 
                                         width = "100%"))
                 ),
                 hr(),
                 plotlyOutput("plot_bilan_chrono", height = "600px")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  get_alt_sim <- reactive({ req(input$fichier_rds_2); readRDS(input$fichier_rds_2) })
  ...
  etgs_shape <- reactive({
    tryCatch({
      shp <- st_read("SIG/ETG.gpkg", quiet = TRUE)
  ...
  output$plot_calibration <- renderPlotly({
    ...
    req(df_plot)
    df_alt <- NULL
    if(input$show_alt_meteo) { df_alt <- get_alt_sim()$liste_finale[[nom_etang]] }
    ...
    suppressWarnings(ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x unified", xaxis = list(rangeslider = list(type = "date"))))
  })
  ...
  output$plot_gantt_assec <- renderPlotly({
    ...
    suppressWarnings(ggplotly(p, tooltip = "text") %>% layout(hovermode = "closest") %>% config(displayModeBar = TRUE))
  })
  ...
  # =======================================================
  # ONGLET 7 : ANALYSE STATISTIQUE DE L'ÉVAPORATION (VOLUMES)
  # =======================================================
  ...
  data_evaporation <- reactive({
    ...
    df_analyse <- df1 %>%
      ...
      mutate(
        ...
        # Filtre strict : Pas de pluie, pas d'action humaine, météo déficitaire, et la sonde voit une baisse
        Est_Jour_Evap = (RR_lisse <= 0.5 & Intervention_sur_fenetre == 0 & Deficit_Meteo_Abs > 0 & Delta_V_Reel_lisse < 0)
      ) %>% 
      select(dat, P_ETP, Deficit_Meteo_Abs, Est_Jour_Evap, Perte_Terrain_Abs, Perte_Mod1_lisse)
    ...
    if (!is.null(df2)) {
      ...
      df_analyse <- df_analyse %>% 
        left_join(df2_sub, by = "dat") %>%
        mutate(Perte_Mod2_lisse = Perte_Mod2_lisse)
    } else {
      df_analyse <- df_analyse %>% mutate(Perte_Mod2_lisse = NA)
    }
  ...
  output$plot_evap_journalier <- renderPlotly({
    ...
    p <- ggplot(df, aes(x = dat)) +
      # Points Terrain
      geom_point(aes(y = Perte_Terrain_Abs, color = "Sonde (Terrain)", text = paste("Date:", dat, "<br>Baisse Réelle:", round(Perte_Terrain_Abs,1), "m³")), size = 3, alpha = 0.8) +
      # Ligne et points Modèle 1
      geom_line(aes(y = Perte_Mod1_lisse, color = "Scénario 1 (Base)"), linewidth = 1, alpha = 0.6) +
      geom_point(aes(y = Perte_Mod1_lisse, color = "Scénario 1 (Base)", text = paste("Date:", dat, "<br>Perte Modèle 1:", round(Perte_Mod1_lisse,1), "m³")), size = 2, shape = 17, alpha = 0.8)
    
    if(any(!is.na(df$Perte_Mod2_lisse))) {
      p <- p + geom_point(aes(y = Perte_Mod2_lisse, color = "Scénario 2 (Comparaison)", text = paste("Date:", dat, "<br>Perte Modèle 2:", round(Perte_Mod2_lisse,1), "m³")), size = 2, shape = 15, alpha = 0.8)
    }
    
    p <- p + theme_minimal() + 
      labs(title = paste("Volumes d'eau perdus (fenêtre de", input$lissage_jours_evap, "jours)"), x = "Date", y = "Volume Perdu (m³)", color = "Légende") +
      scale_color_manual(values = c("Sonde (Terrain)" = "#e67e22", "Scénario 1 (Base)" = "#2980b9", "Scénario 2 (Comparaison)" = "#8e44ad"))
    
    suppressWarnings(ggplotly(p, tooltip = "text") %>% layout(hovermode = "closest"))
  })
  
  output$table_evap_resume <- renderDT({
    df <- data_evaporation()
    req(input$dates_evap)
    
    df <- df %>% filter(dat >= input$dates_evap[1] & dat <= input$dates_evap[2])
    df_evap <- df %>% filter(Est_Jour_Evap == TRUE)
    
    nb_jours <- nrow(df_evap)
    
    # Petite fonction pour arrondir proprement
    f_rnd <- function(x) round(x, 1)
    
    res <- tibble(
      `Source des données` = c("Terrain (Sonde)", "Scénario 1 (Base)", "Scénario 2 (Comparaison)"),
      `Nb Périodes Analysées` = c(nb_jours, nb_jours, nb_jours),
      `Moyenne Perdue (m³)` = c(
        f_rnd(mean(df_evap$Perte_Terrain_Abs, na.rm=TRUE)),
        f_rnd(mean(df_evap$Perte_Mod1_lisse, na.rm=TRUE)),
        f_rnd(mean(df_evap$Perte_Mod2_lisse, na.rm=TRUE))
      ),
      `Variance (m³²)` = c(
        f_rnd(var(df_evap$Perte_Terrain_Abs, na.rm=TRUE)),
        f_rnd(var(df_evap$Perte_Mod1_lisse, na.rm=TRUE)),
        f_rnd(var(df_evap$Perte_Mod2_lisse, na.rm=TRUE))
      ),
      `Écart-Type (m³)` = c(
        f_rnd(sd(df_evap$Perte_Terrain_Abs, na.rm=TRUE)),
        f_rnd(sd(df_evap$Perte_Mod1_lisse, na.rm=TRUE)),
        f_rnd(sd(df_evap$Perte_Mod2_lisse, na.rm=TRUE))
      ),
      `Biais Moyen (Mod - Obs) (m³)` = c(
        NA, 
        f_rnd(mean(df_evap$Perte_Mod1_lisse - df_evap$Perte_Terrain_Abs, na.rm=TRUE)),
        f_rnd(mean(df_evap$Perte_Mod2_lisse - df_evap$Perte_Terrain_Abs, na.rm=TRUE))
      ),
      `RMSE (m³)` = c(
        NA,
        f_rnd(sqrt(mean((df_evap$Perte_Mod1_lisse - df_evap$Perte_Terrain_Abs)^2, na.rm=TRUE))),
        f_rnd(sqrt(mean((df_evap$Perte_Mod2_lisse - df_evap$Perte_Terrain_Abs)^2, na.rm=TRUE)))
      )
    )
  ...
  # =======================================================
  # ONGLET 8 : BILAN CHRONOLOGIQUE GLOBAL (EN MM + PLUIE)
  # =======================================================
  ...
  output$plot_bilan_chrono <- renderPlotly({
    ...
    # Filtre sur les dates sélectionnées
    if (!is.null(input$dates_bilan_global)) {
      df_all <- df_all %>% filter(dat >= input$dates_bilan_global[1] & dat <= input$dates_bilan_global[2])
    }
    
    # Agrégation journalière pour toute la chaîne
    df_chrono <- df_all %>%
      group_by(dat) %>%
      summarise(
        Stock_Estime = sum(BF, na.rm = TRUE),
        Vmax_Tout_Etang = sum(Vmax, na.rm = TRUE),
        Besoin_Evolage = sum(ifelse(is.na(Statut_Simu) | Statut_Simu != "Assec", Vmax, 0), na.rm = TRUE),
        Surface_Totale_BV = sum(Surface_BV, na.rm = TRUE), # Somme des surfaces des bassins versants (ha)
        RR = mean(RR, na.rm = TRUE), # La pluie est la même partout, la moyenne donne la pluie du jour
        .groups = "drop"
      )
    
    # Lissage selon le pas de temps choisi
    req(input$time_step_bilan)
    if (input$time_step_bilan != "day") {
      df_chrono <- df_chrono %>%
        mutate(dat = lubridate::floor_date(dat, unit = input$time_step_bilan)) %>%
        group_by(dat) %>%
        summarise(
          Stock_Estime = mean(Stock_Estime, na.rm = TRUE),          # Moyenne du stock
          Vmax_Tout_Etang = mean(Vmax_Tout_Etang, na.rm = TRUE),
          Besoin_Evolage = mean(Besoin_Evolage, na.rm = TRUE),
          Surface_Totale_BV = mean(Surface_Totale_BV, na.rm = TRUE),
          RR = sum(RR, na.rm = TRUE),                               # SOMME de la pluie sur la période
          .groups = "drop"
        )
    }
    
    # Conversion des m³ en mm : Volume (m³) / (Surface (ha) * 10)
    df_chrono <- df_chrono %>%
      mutate(
        Stock_mm = Stock_Estime / (Surface_Totale_BV * 10),
        Vmax_mm = Vmax_Tout_Etang / (Surface_Totale_BV * 10),
        Besoin_mm = Besoin_Evolage / (Surface_Totale_BV * 10)
      )
    
    p <- ggplot(df_chrono, aes(x = dat)) +
      # 1. Pluie (Histogramme en arrière-plan)
      geom_col(aes(y = RR, fill = "Pluviométrie (mm)"), alpha = 0.25) +
      
      # 2. Volume max total (Ligne orange constante)
      geom_line(aes(y = Vmax_mm, color = "Volume max tout étangs"), linewidth = 1) +
      
      # 3. Besoin en eau Vmax évolage (Ligne noire en escalier)
      geom_line(aes(y = Besoin_mm, color = "Besoin en eau Vmax évolage"), linewidth = 1) +
      
      # 4. Estimation du stock (Ligne bleue claire fluctuante)
      geom_line(aes(y = Stock_mm, color = "Estimation du stock des étangs"), linewidth = 1.2, alpha = 0.8) +
      
      scale_color_manual(values = c(
        "Volume max tout étangs" = "#e67e22",        # Orange
        "Besoin en eau Vmax évolage" = "#2c3e50",    # Noir/Gris foncé
        "Estimation du stock des étangs" = "#85c1e9" # Bleu clair
      )) +
      scale_fill_manual(values = c("Pluviométrie (mm)" = "#3498db")) +
      
      theme_minimal() +
      labs(
        title = "Bilan hydrique estimé de la chaîne d'étangs de Chalamont",
        x = "Date",
        y = "Bilan en mm (Litre/m² BV)",
        color = "",
        fill = ""
      ) +
      theme(legend.position = "bottom")
    
    suppressWarnings(ggplotly(p, dynamicTicks = TRUE) %>% layout(
      hovermode = "x unified", 
